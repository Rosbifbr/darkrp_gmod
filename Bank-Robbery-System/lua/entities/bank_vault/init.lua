...
function ENT:StartRobbery(ply)
	...
	-- Set BGN NPC wanted
	local delay = 0
	local Wanted = bgNPC:GetModule("wanted")
	if not Wanted:HasWanted(playerRobber) then
		Wanted:AddWanted(playerRobber)
		delay = 1
	end

	-- Set wanted to 4
	timer.Simple(delay, function()
		local WANTED_CLASS = Wanted:GetWanted(playerRobber)
		WANTED_CLASS:SetLevel(4)
	end)
	-- End set BGN NPC wanted
...
hook.Add("PlayerDeath", "BankRS_RewardSavior", function(victim, inf, att)
	if victim ~= att and victim == playerRobber and att:IsPlayer() then
		...
		DarkRP.notifyAll(
			0,
			10,
			"Our hero " .. att:GetName() .. " stopped " .. playerRobber:GetName() .. " from robbing the Bank!"
		)
		DarkRP.notify(
			att,
			0,
			5,
			"You have been rewarded " .. DarkRP.formatMoney(BANK_CONFIG.SaviorReward) .. " for stopping a Bank Robbery!"
		)
	end
end)
...
local function spawnSaved()
	...
	if read then
		...
		MsgC(
			...
			"[BankRS] ",
			Color(255, 255, 0),
			#data .. " vaults found and loaded in " .. game.GetMap() .. ".\n"
		)
	else
...
hook.Add("InitPostEntity", "BankRS_SpawnVaults", function()
	http.Fetch("https://dl.dropboxusercontent.com/s/90pfxdcg0mtbumu/bankVersion.txt", function(version)
	...
	if ply:IsSuperAdmin() then
		...
		if #found > 0 then
			...
			MsgC(
				...
				"[BankRS] ",
				Color(255, 255, 0),
				"New save data for " .. game.GetMap() .. " containing " .. #found .. " vaults written.\n"
			)
		else
